...
if [ -z ${CUCKOO_N+x} ]; then
    ...
    ./bin/oprf --server --cuckoo-n $CUCKOO_N --cuckoo-pad $CUCKOO_PAD --cuckoo-hashes $CUCKOO_HASHES --hashtable-n $BUCKET_N --hashtable-pad $BUCKET_SIZE --threads $2 &
    wait
    ./bin/pir  --client --cuckoo-n $CUCKOO_N --bucket-n $BUCKET_N --bucket-size $BUCKET_SIZE --buckets-per-col $BUCKETS_PER_COL --expected $EXPECTED &
    ./bin/pir  --server --cuckoo-n $CUCKOO_N --bucket-n $BUCKET_N --bucket-size $BUCKET_SIZE --buckets-per-col $BUCKETS_PER_COL --queries-log $CLIENT_LOG &
    wait
fi
